...
| 9   | Booking Cancellation / Validation | Users can cancel bookings for sessions that have already started |  Medium  |  Open  | Reproduction: As a regular user, book a session. When the session start time has passed, navigate to "My Bookings" section. The "Cancel" button is still active and allows cancellation even though the session has already started.<br>Expected: Users should not be able to cancel bookings for sessions that have already started (past sessions should either be hidden from My Bookings or the Cancel button should be disabled).                                                   |
